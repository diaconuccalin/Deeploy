...
> **Branch:** DemoTinyViT_Tiled_Siracusa (Tiled Conv2D improvements with TinyViT demo)

---

## Branch Context

This branch (`DemoTinyViT_Tiled_Siracusa`) focuses on:
- **Enhanced Conv2D tiling**: Improved input tiling constraints for 2D convolutions
- **Edge case handling**: Better handling of padding and bordering regions in tiled operations
- **TinyViT demonstration**: Includes `testFloatDemoTinyViT` test showcasing tiled inference on a vision transformer model
- **PULP platform optimizations**: Float and depthwise convolution tiling improvements for Siracusa

**Key improvements over base:**
- Input tiling support on x and y axes for PULP FP Conv2D (previously channel-wise only)
- Fixed overflow issues at edges of input tensors during tiling
- Improved absolute offset computation for padding in multi-level memory hierarchies
